...
document.getElementById('form-information').addEventListener('submit', async function (event) {
    ...
    var selectedTopics = [];
    document.querySelectorAll('input[name="topics"]:checked').forEach(function (checkbox) {
        selectedTopics.push(checkbox.value);
    });

    // Ensure at least one topic is selected
    if (selectedTopics.length === 0) {
        return showSnackBar("Please select at least one topic.");
    }
    formData.selectedTopics = selectedTopics;
...
async function register(formData, serverTime) {
    try {
        ...
        var request = await new BeeStore({ path: `SymposiumApplications` }).addDoc(inputData);
